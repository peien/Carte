    ...
    if (self.isContactCellClick) {
        ...
        if (phones.count > 0) {
            for (int i = 0; i < phones.count; i++) {
                ...
                [act showInView:_viewController.navigationController.tabBarController.view];
            }
        }
        
    }else{
        if (_card.mobile0||_card.tele0) {
            [self doInSelf:act addTitle:_card.mobile0];
            [self doInSelf:act addTitle:_card.mobile1];
            [self doInSelf:act addTitle:_card.mobile2];
            [self doInSelf:act addTitle:_card.tele0];
            [self doInSelf:act addTitle:_card.tele1];
            [self doInSelf:act addTitle:_card.tele2];
            [act showInView:_viewController.navigationController.tabBarController.view];
        }    
    }
...
- (void)doInSelf:(UIActionSheet *)act addTitle:(NSString *)title
{
    if (!title) {
        return;
    }
    [act addButtonWithTitle:title];
}
//发短信
- (void)sendMessage
{
    ...
    if (self.isContactCellClick) {
        ...
        if (phones.count > 0) {
            ...
            [actS showInView:_viewController.navigationController.tabBarController.view];
        }
        
    }else{
        if (_card.mobile0) {
            [self doInSelf:actS addTitle:_card.mobile0];
            [self doInSelf:actS addTitle:_card.mobile1];
            [self doInSelf:actS addTitle:_card.mobile2];
...
- (void)sendMail{
    ...
    if (mfVC != nil) {
        if ([mfVC canSendMail]) {
            ...
            NSArray *emails = [NSArray arrayWithObjects:_card.email0,_card.email1,_card.email2,nil];
