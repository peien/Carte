...
@implementation AGHeadImageView
...
- (void)initialize
{
    pinchRecognizer = [[UIPinchGestureRecognizer alloc]initWithTarget:self action:@selector(scaGesture:)];
    ...
    [ratioView addGestureRecognizer:pinchRecognizer];
...
- (void)resetRatioControls
{
    ...
    CGSize imageSizeAfterRotation = CGSizeAbsolute2([self sizeForRotatedImage:self.imageView.image]);
...
- (CGRect)imageFrameFromImageViewWithAspectFitMode:(UIImageView *)theImageView
{
    ...
    CGSize imageSize = CGSizeAbsolute2([self sizeForRotatedImage:self.imageView.image]);
...
- (UIImage *)rotatedImage:(UIImage *)imageToRotate
{
    ...
    CGSize absoluteOutputSize = CGSizeAbsolute2(outputSize);
...
- (void)panGesture:(UIPanGestureRecognizer *)recognizer
{
   ...
   // CGPoint center = CGPointMake(0, 0);
    ...
    CGRect rectPro = ratioView.frame;
    
    CGPoint center =  ratioView.center;
    CGPoint centerPro = CGPointMake(center.x+translation.x, center.y + translation.y);
    if (center.x - rectPro.size.width/2.0<0.0) {
        centerPro.x = rectPro.size.width/2.0;
    }
    
    if (center.x + rectPro.size.width/2.0 > overlayView.frame.size.width) {
        centerPro.x = overlayView.frame.size.width - rectPro.size.width/2.0;
    }
    if (center.y - rectPro.size.height/2.0<0.0) {
        centerPro.y = rectPro.size.height/2.0;
    }
    
    if (center.y + rectPro.size.height/2.0 > overlayView.frame.size.height) {
        centerPro.y = overlayView.frame.size.height - rectPro.size.height/2.0;
    }
//    
//    
...
-(void)scaGesture:(id)sender {
    ...
    CGRect rectPro = ratioView.frame;
    CGPoint center = ratioView.center;
    float widthPro = rectPro.size.width*scale;
    float pointXPro = rectPro.origin.x - (widthPro - rectPro.size.width)/2.0;
    
    float heightPro = rectPro.size.height*scale;
    float pointYPro = rectPro.origin.y - (heightPro - rectPro.size.height)/2.0;
    
    if (pointXPro<=0.0) {
        
       // float scalPro3 = (widthPro - (-pointXPro)*2) / rectPro.size.width ;
        float scalPro3 = (center.x)*2 / rectPro.size.width;
        widthPro = rectPro.size.width*scalPro3;
        pointXPro = rectPro.origin.x - (widthPro - rectPro.size.width)/2.0;
        heightPro = rectPro.size.height*scalPro3;
        pointYPro = rectPro.origin.y - (heightPro - rectPro.size.height)/2.0;
    }
    if (pointXPro + widthPro >= overlayView.frame.size.width) {
        float scalPro3 = (overlayView.frame.size.width - (center.x))*2 / rectPro.size.width;
        widthPro = rectPro.size.width*scalPro3;
        pointXPro = rectPro.origin.x - (widthPro - rectPro.size.width)/2.0;
        heightPro = rectPro.size.height*scalPro3;
        pointYPro = rectPro.origin.y - (heightPro - rectPro.size.height)/2.0;
    ...
    if (pointYPro<=0.0) {
        ...
        // float scalPro3 = (widthPro - (-pointXPro)*2) / rectPro.size.width ;
        float scalPro3 = center.y*2 / rectPro.size.height;
        widthPro = rectPro.size.width*scalPro3;
        pointXPro = rectPro.origin.x - (widthPro - rectPro.size.width)/2.0;
        heightPro = rectPro.size.height*scalPro3;
        pointYPro = rectPro.origin.y - (heightPro - rectPro.size.height)/2.0;
    }
    if (pointYPro + heightPro >= overlayView.frame.size.height) {
        float scalPro3 = (overlayView.frame.size.height - (center.y))*2 / rectPro.size.height;
        widthPro = rectPro.size.width*scalPro3;
        pointXPro = rectPro.origin.x - (widthPro - rectPro.size.width)/2.0;
        heightPro = rectPro.size.height*scalPro3;
        pointYPro = rectPro.origin.y - (heightPro - rectPro.size.height)/2.0;
    ...
    ratioView.frame = CGRectMake(pointXPro, pointYPro, widthPro, heightPro);
    //
    //
