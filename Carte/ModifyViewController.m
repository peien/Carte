...
#define textChanging NSLocalizedString(@"正在修改密码请稍侯...", @"")
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    self.barRightStr = @"完成";
    ModifyViewController *weakSelf = self;
    self.barRightAction = ^(){
...
- (void)changePassword{
    ...
    dicParam[@"oldPassword"] = oldPass;
    dicParam[@"newPassword"] = newPass;
    [[NetClient sharedClient]doInselfPath:@"post" methodPath:@"mobile/user/password" parameters:dicParam success:^(NSMutableDictionary *responseDict) {
        [KHHUser shareInstance].password = newPass;
        [_hud hide:YES];
        [[[UIAlertView alloc]initWithTitle:@"修改密码成功" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil] show];
    } failure:^(NSMutableDictionary *responseDict) {
        [_hud hide:YES];
        [[[UIAlertView alloc]initWithTitle:@"修改密码失败" message:responseDict[@"note"] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil] show];
    }];
}
...
//    [_hud hide:YES];
//
//    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"密码修改失败" message:userInfo[@"note"] delegate:nil cancelButtonTitle:textOK otherButtonTitles:nil, nil];
//    [alert show];
//}
